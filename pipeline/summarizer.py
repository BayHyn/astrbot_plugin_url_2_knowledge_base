...
async def _summarize_map_reduce(
    ...
    """
    logger.info(f"  - Activating map-reduce for {len(chunks)} chunks...")
    ...
    logger.info(f"  - Map phase: Created {len(super_chunk_tasks)} intermediate summary tasks.")
    ...
    if not valid_intermediate_summaries:
        logger.error("Failed to generate any intermediate summaries in map-reduce.")
    ...
    logger.info(f"  - Reduce phase: Summarizing {len(valid_intermediate_summaries)} intermediate summaries.")
...
async def generate_summaries(
    ...
    logger.info(f"Found {len(clusters)} topics to summarize and {len(noise_points)} noise points.")
    ...
    for topic_id, chunks in sorted(clusters.items()):
        ...
        if len(chunk_texts) > summarization_chunk_threshold:
            logger.info(f"  - Topic {topic_id} has {len(chunk_texts)} chunks, using map-reduce.")
            task = _summarize_map_reduce(chunk_texts, summarize_llm_service, rate_limiter)
        else:
            logger.info(f"  - Summarizing topic {topic_id} ({len(chunk_texts)} chunks) directly...")
            full_topic_text = "\n\n".join(chunk_texts)
    ...
    for topic_id, result in zip(topic_summary_tasks.keys(), topic_summary_results):
        if isinstance(result, Exception):
            logger.error(f"  - Summarization for topic {topic_id} failed: {result}")
            topic_summaries[topic_id] = f"Error summarizing topic: {result}"
        else:
            topic_summaries[topic_id] = result
            logger.info(f"  - Summary for topic {topic_id} generated.")

    # --- Step 2: Generate Overall Summary (with fallback) ---
    overall_summary = ""
    if not clusters:
        logger.warning("⚠️ No valid topics found. Activating fallback: summarizing all text chunks via map-reduce.")
        ...
        logger.info("Generating overarching summary from topic summaries...")
        ...
        logger.info("✅ Overarching summary generated.")
